...
load (const char *_path, char **argv, void (**eip) (void), void **esp)
{
  ...
  if (file_read (t->bin_file, &ehdr, sizeof ehdr) != sizeof ehdr
  ...
  for (i = 0; i < ehdr.e_phnum; i++)
    {
      ...
      if (file_ofs < 0 || file_ofs > file_length (t->bin_file))
        goto done;
      file_seek (t->bin_file, file_ofs);

      if (file_read (t->bin_file, &phdr, sizeof phdr) != sizeof phdr)
        goto done;
      ...
      switch (phdr.p_type)
        ...
        case PT_SHLIB:
          goto done;
        case PT_LOAD:
          if (validate_segment (&phdr, t->bin_file))
            {
              ...
              if (!load_segment (t->bin_file, file_page, (void *) mem_page,
      ...
      if (page->frame != NULL)
        {
          ...
          page->private = false;
          ok = init_cmd_line (page->frame->base, page->addr, argv, esp);
