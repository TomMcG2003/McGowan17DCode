...
static int sys_creat (const char *ufile, unsigned initial_size);
...
/* Controls access to critical sections that manipulate the file system. */
...
/* Create system call. */
static int
sys_creat (const char *ufile, unsigned initial_size)
{
  ...
  while (size > 0)
    {
      ...
      if (handle == STDOUT_FILENO)
        {
          putbuf ((const uint8_t *) usrc, write_amt);
          retval = write_amt;
        }
      else
