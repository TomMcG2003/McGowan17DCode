...
int testGrayChild(char *path) {
    ...
    if ((object) && (strncmp(object->name, lastComponent, GRAY_NAME_SIZE) == 0)) {
        ...
        if (object) {
            printf("Object is non-NULL\n");
            printf("object component name %s\n", object->name);
            printf("Looking for %s\n", lastComponent);
        } else {
            printf("Sadly, the object came back NULL\n");
...
int main(int argc, char *argv[]) {
    char content[GRAY_MAX_FILESIZE];
    ...
    */
    ...
    int count = gray_read(file21, content, 0, 33);
    printf("Before Write Value: *%s* with count of %d\n", content, count);
    ...
    printf("After write:\n");
    printObjectContent(file21);
    count = gray_read(file21, content, 2, 33);
    printf("Read: *%s* with count = %d\n", content, count);
